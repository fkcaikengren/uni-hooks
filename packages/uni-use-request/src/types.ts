...
export interface UseRequestBasicOptions<TData, TParams extends unknown[]> {
  ...
  /**
   * Whether to continue polling when the page is hidden. If set to false,
   * polling will be temporarily paused when the page is hidden, and resume when the page is visible again.
   */
  ...
  /**
   * A unique ID of the request. If `cacheKey` is set, we will enable the caching mechanism.
   * The data of the same `cacheKey` is globally synchronized.
   */
  cacheKey?: string
   ...
   * - In the custom cache mode, `cacheTime` and `clearCache` are useless,
   * please implement it yourself according to the actual situation.
   ...
   * If not set, the simple exponential backoff algorithm will be used by default,
   * taking `1000 * 2 ** retryCount`, that is, waiting for 2s for the first retry,
   * and 4s for the second retry. By analogy, if it is greater than 30s, take 30s.
   */
