...
import { watchPausable } from '@caikengren/uni-hooks-shared';
import { tryOnMounted } from '@caikengren/uni-hooks-shared';
import { tryOnScopeDispose } from '@caikengren/uni-hooks-shared';
import { WatchPausableReturn } from '@caikengren/uni-hooks-shared';
import type { ConfigurableFlush, RemovableRef } from '@caikengren/uni-hooks-shared';
import { useUniInterceptor } from '../useUniInterceptor';
import { isNil } from '@caikengren/uni-hooks-shared';
import type { ConfigurableEventFilter } from '@caikengren/uni-hooks-shared';
 ...
 * @param {'pre'|'post'|'sync'} [options.flush='pre'] - 控制何时将值写入存储
 * @param {boolean} [options.deep=true] - 是否深度监听对象变化
 * @param {boolean} [options.listenToStorageChanges=true] - 是否监听外部存储变化
 * @param {boolean|Function} [options.mergeDefaults=false] - 是否/如何合并默认值和存储值
 * @param {boolean} [options.shallow=false] - 是否使用shallowRef
 * @param {Function} [options.onError] - 错误回调函数
 * @param {boolean} [options.initOnMounted=false] - 是否在组件挂载后读取存储
